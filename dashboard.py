...
/* The update feed runs longer than the sidebar (deadlines, capped at 8), so
   once the feed scrolls past it the sidebar would just end, leaving a tall
   blank column beside more update cards. Sticky keeps it in view while the
   feed scrolls — align-items:start above matters here: it keeps the
   ...
   before its own bottom reaches the row's bottom and it scrolls away
   normally. No flex:1 stretch anywhere: every attempt at growing a trailing
   card or panel to close the height gap just relocated the same dead space
   into whichever element absorbed it — an empty bordered box on
   quickcontact, a blank slab inside .p-updates, a gap inside the deadlines
   panel. Sticky sidesteps the mismatch instead of papering over it. */
