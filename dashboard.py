...
/* Advisory callout: the tool tracks the change, the team helps you act on it.
   A light card in the callout family (like .roundup and the "Read this first"
   notice), deliberately NOT the navy .sectionband strip vocabulary used for
...
/* Multi-list callout: shown above the result cards when one screened name
   hits more than one list, so breadth of coverage is legible at a glance. */
.sdnhitsum{font-size:12px;margin:0 0 8px;padding:6px 9px;line-height:1.5;
  background:var(--raised);border-left:3px solid var(--brand);border-radius:3px}
.sdnhitsum strong{color:var(--ink)}
.sdnhitsum .sdnhitlists{color:var(--ink-muted)}
.sdnbulkrow .sdnonlists{font-weight:600;color:var(--crit)}
...
/* Same two-tone sweep button as #showmore / the footer Subscribe: navy body,
   8px lime left trim, white text, lime fill sliding in on hover. */
.roundup a{display:inline-block;font:inherit;font-size:13px;font-weight:600;
  ...
  background-size:200% 100%;background-position:right bottom;
  transition:background-position .5s ease}
.roundup a:hover{background-position:left bottom}
...
if (sdnListQ) {
  ...
  function renderOne(query) {
    ...
    // Call out when one screened name lands on more than one list — that
    // breadth is the point of screening every list at once.
    const srcs = [...new Set(m.map(x => x.row[4]))];
    const hitSum = srcs.length >= 2
      ? `<p class="sdnhitsum"><strong>On ${srcs.length} lists</strong> `
        + `<span class="sdnhitlists">&mdash; ${m.length} matching entries: `
        + `${srcs.map(esc).join(' &middot; ')}</span></p>`
      : '';
    results.innerHTML = shown.length ? hitSum + shown.map(rowHtml).join('')
  ...
  function runBulk() {
    ...
    loadIndex().then(loadSam).then(() => {
      ...
      results.innerHTML = names.map(name => {
        ...
        if (m.length) hits++;
        const bsrcs = [...new Set(m.map(x => x.row[4]))];
        const onLists = bsrcs.length >= 2
          ? ` <span class="sdnonlists">on ${bsrcs.length} lists</span>` : '';
...
def coverage_panel(store):
    ...
    return (
        ...
        'Bank, ADB and AfDB debarment lists and FinCEN Section 311 special '
        'measures &mdash; and '
...
def sdn_panel(today):
    ...
    full_search = (
        ...
        '<input type="range" id="sdnminscore" min="50" max="100" step="5" value="95" '
        'aria-label="Minimum name score — lower to see near matches">'
        '<span class="sdnscoreval" id="sdnminscoreval">95</span>'
        '</label>'
    ...
    return (
        ...
        'and EU consolidated lists, the World Bank, ADB and AfDB debarment lists '
        '(with MDB cross-debarments) and FinCEN Section 311 special measures '
...
<html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>KleaRance: regulatory update tracker for community banks, credit unions and fintechs</title>
...
<!-- Tab, bookmark and home-screen icons. Generated by make_icons.py; the paths
     ...
     ?v=5 cache-busts every icon URL, manifest included -- Android's "Install
...
<link rel="icon" type="image/png" href="icon-32.png?v=5" sizes="32x32">
<link rel="icon" type="image/png" href="icon-16.png?v=5" sizes="16x16">
...
<meta property="og:site_name" content="KleaRance">
<meta property="og:title" content="KleaRance: regulatory update tracker for community banks, credit unions and fintechs">
...
<meta name="twitter:title" content="KleaRance: regulatory update tracker for community banks, credit unions and fintechs">
  ...
  <div class="hero-inner">
    <div class="hero-titleblock">
      <h1 class="hero-word"><svg viewBox="0 0 40 34" aria-hidden="true">
        ...
        </svg><span class="kr kr-k">K<span class="kr-divider" aria-hidden="true"></span></span><span class="hero-word-rest">lea</span><span class="kr kr-r">R</span><span class="hero-word-rest">ance</span><span class="sr-only">: regulatory update tracker for community banks, credit unions and fintechs</span></h1>
...
<div class="advisory-band">
  <span class="ab-eyebrow">From the advisory team</span>
...
<div class="quickcontact">
  ...
  <div class="qc-text">
    ...
    <p class="qc-blurb">Not sure what an update here means for your institution? Email me.</p>
    <div class="qc-icons">
